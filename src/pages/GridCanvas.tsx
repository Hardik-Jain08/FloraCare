		...
		const calculateVisibleLineDefs = () => {
			...
			Array(vertLines)
				...
				.forEach((n, i) => {
					const x =
						Math.round(
							firstVertLineX + i * size - leftRef.current
						) + 0.5;
					result.push({
						start: { x, y: 0 },
			...
			Array(horizLines)
				...
				.forEach((n, i) => {
					const y =
						Math.round(firstHorizLineY + i * size + yOffset) + 0.5;
					if (y < 0) return;
					result.push({
						start: { x: 0, y },
	...
	return (
		...
		>
			...
			>
				...
				<g
					stroke={color}
					mask={p.maskId ? `url(#${maskId})` : undefined}
				>
					{lines.map((def, i) => (
						<line
							key={i}
		...
		get scale() {
			return (
				...
				Math.max(
					...
					(1 -
						Math.abs(
							transformsRef.current.x + transformsRef.current.y
						) /
							(p.containerWidthRef.current! +
								p.containerHeightRef.current!)) *
						0.5
				)
			);
		},
